...
import random, sys, time, math, pygame, shelve
...
STARTSIZE = 49       # how big the player starts off
WINSIZE = 250        # how big the player needs to be to win
...
SQUIRRELMAXSPEED = 6 # fastest squirrel speed
...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, SQUIR_IMG, GRASSIMAGES, SQUIRREL, SCORE_VALUE, counter
...
def runGame():
    ...
    playerObj = {'surface': pygame.transform.scale(pygame.image.load('Fox.png'), (STARTSIZE, STARTSIZE)),
                 'facing': LEFT,
                 'size': STARTSIZE,
                 'x': HALF_WINWIDTH,
                 'y': HALF_WINHEIGHT,
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                counter -= 1
                text = BASICFONT.render(str(counter), True, (0, 128, 0))
                if counter == 0:
                ...
                if event.key in (K_UP, K_w):
                    ...
                    if playerObj['facing'] != LEFT: # change player image
                        playerObj['surface'] = pygame.transform.scale(pygame.image.load('Fox.png'), (playerObj['size'], playerObj['size']))
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        if playerObj['health'] == 0 or counter < 0.9:
...
def makeNewSquirrel(camerax, cameray):
    sq = {}
    generalSizeX = random.randint(7, 25)
    ...
    sq['bounce'] = 0
    sq['bouncerate'] = random.randint(10, 15)
    sq['bounceheight'] = random.randint(10, 30)
    return sq
